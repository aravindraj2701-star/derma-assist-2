...
def validate_image(file_bytes: bytes, filename: str, max_size_bytes: int) -> Image.Image:
    ...
    try:
        ...
        img = Image.open(io.BytesIO(file_bytes))
        img = img.convert("RGB")
        # Memory safety: restrict oversized dimensions to max 512px to prevent memory spikes on Render free tier
        if max(img.size) > 512:
...
def image_to_optimized_base64(img: Image.Image, max_dim: int = 480, quality: int = 82) -> str:
    """Convert a PIL Image to a lightweight, responsive JPEG base64 string (~40-80KB)."""
    buffer = io.BytesIO()
